    ...
    async def run(self) -> None:
        ...
        while self.is_running or not self.pipeline_manager.save_queue.empty():
            try:
                ...
                print(f"{self.worker_name}: Saving image {index} started")
                ...
                try:
                    ...
                    print(f"{self.worker_name}: Saving image {index} finished - {time.time() - start_time:.2f}s")

                except Exception as e:
                    self.pipeline_manager.stats.errors += 1
                    print(f"{self.worker_name}: Error saving image {index}: {e}")
    ...
    async def _save_single_image(self, image: np.ndarray, file_path: str) -> None:
        ...
        try:
            ...
            if success:
                ...
                print(f"Failed to encode image for {file_path}")
        except Exception as e:
